...
exports.lookup_path =
  ...
  exports.UnverifiedCertificateError =
    void 0;
...
class Certificate {
  ...
  async verify() {
    ...
    const res = await bls_1.blsVerify(
      new Uint8Array(key),
      new Uint8Array(sig),
      new Uint8Array(msg),
    );
  ...
  async _checkDelegation(d) {
    ...
    if (!lookup) {
      throw new Error(
        `Could not find subnet key for subnet 0x${buffer_1.toHex(d.subnet_id)}`,
      );
    }
    return lookup;
  }
...
const DER_PREFIX = buffer_1.fromHex(
...
async function reconstruct(t) {
  switch (t[0]) {
    ...
    case 4 /* Pruned */:
      return t[1];
    case 3 /* Leaf */:
      return request_id_1.hash(
        buffer_1.concat(domain_sep('ic-hashtree-leaf'), t[1]),
      );
    case 2 /* Labeled */:
      return request_id_1.hash(
        buffer_1.concat(
