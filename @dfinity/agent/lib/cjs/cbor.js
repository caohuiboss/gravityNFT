  ...
  encode(v) {
    ...
    if (v > BigInt(0)) {
      return cbor.value.tagged(
        2,
        cbor.value.bytes(buffer_1.fromHex(v.toString(16))),
      );
    } else {
      return cbor.value.tagged(
        3,
        cbor.value.bytes(buffer_1.fromHex((BigInt('-1') * v).toString(16))),
      );
    }
  }
}
const serializer =
  simple_cbor_1.SelfDescribeCborSerializer.withDefaultEncoders(true);
