...
class RecordClass extends ConstructType {
  constructor(fields = {}) {
    ...
    this._fields = Object.entries(fields).sort(
      (a, b) => hash_1.idlLabelToId(a[0]) - hash_1.idlLabelToId(b[0]),
    );
  }
  accept(v, d) {
  ...
  _buildTypeTableImpl(T) {
    ...
    const fields = this._fields.map(([key, value]) =>
      buffer_1.concat(
        leb128_1.lebEncode(hash_1.idlLabelToId(key)),
        value.encodeType(T),
      ),
    );
  ...
  decodeValue(b, t) {
    ...
    for (const [hash, type] of record._fields) {
      if (
        ...
        hash_1.idlLabelToId(this._fields[idx][0]) !== hash_1.idlLabelToId(hash)
      ) {
        // skip field
...
function decodePrincipalId(b) {
  ...
  const len = Number(leb128_1.lebDecode(b));
  return principal_1.Principal.fromUint8Array(
    new Uint8Array(leb128_1.safeRead(b, len)),
  );
}
/**
...
class FuncClass extends ConstructType {
  ...
  _buildTypeTableImpl(T) {
    ...
    const args = buffer_1.concat(
      ...this.argTypes.map((arg) => arg.encodeType(T)),
    );
    const retLen = leb128_1.lebEncode(this.retTypes.length);
    const rets = buffer_1.concat(
      ...this.retTypes.map((arg) => arg.encodeType(T)),
    );
    const annLen = leb128_1.lebEncode(this.annotations.length);
    const anns = buffer_1.concat(
      ...this.annotations.map((a) => this.encodeAnnotation(a)),
    );
    T.add(
      this,
      buffer_1.concat(opCode, argLen, args, retLen, rets, annLen, anns),
    );
  }
  decodeValue(b) {
    const x = leb128_1.safeReadUint8(b);
